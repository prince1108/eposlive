...
public class MenuCategoryAdapterForTable extends AbstractMenuCategoryAdapter {
    private Table table;
    ...
    public MenuCategoryAdapterForTable(Activity context, Table table, List<MenuCategory> menuCategories) {
...
//                int totalItems = getTotalItems(menuCategory, realm);
    ...
    private int getTotalItems2(MenuCategory category, Realm realm) {
        /*return category.menuItems.where().notEqualTo("type", Constants.ITEM_TYPE_COMMON).findAll().asFlowable()
                .flatMap(menuItems -> {
                    List<Long> longList = new ArrayList<>(menuItems.size());
                    for(MenuItem menuItem: menuItems) {
                        if(menuItem!=null)
                        longList.add(menuItem.id);
                    }
                    return Flowable.just(longList);
                }).flatMap(longs -> {
                    Long[] arr = longs.toArray(new Long[longs.size()]);
                    return Flowable.just(getItemSize2(arr, realm));
        }).blockingFirst();*/
